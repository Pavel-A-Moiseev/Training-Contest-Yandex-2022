...
//Input
    ...
    for (int i = 0; i<n; i++){
        ...
        for(int j=0; j<4; j++){ //To cut zero-area elements

            if (mas[i].Q[j].height>maxHeight){
                maxHeight = mas[i].Q[j].height;
            }
            if (mas[i].Q[j].width>maxWidth){ 
                maxWidth = mas[i].Q[j].width; 
            }
            if (mas[i].Q[j].height == maxHeight && mas[i].Q[j].width == maxWidth){
                max[j]=i;
            }
        }
...
//Counting
    ...
    for (int j=0; j<4; j++){ //Quarters
        ...
        for (int i = n-1; i>=0; i--){ // Pick a photo
            ...
            } else {mas[i].Q[j].area=0;}

            if (j==3) {
                if (i<min_i){
                    mas[i].totalArea=0;
                } else { mas[i].FtotalArea(); }
            }
